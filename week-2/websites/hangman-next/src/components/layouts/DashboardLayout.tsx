...
const DashboardLayout = ({ children, path }: TProfileLayout) => {
  const { user, isLoaded } = useUser();

  const profileLinks = NavbarDropdownLinks(25);

  return (
    <div
      className={`grid-4 gap-5 mb-5 lg:mb-0 rounded-md p-5 mt-20`}
    >
      <div className="hidden lg:flex flex-col gap-10">
        <div className="flex-col-3">
          <ProfileImage status={isLoaded} data={user} />
          <div className="flex-col-1">
            <Username status={isLoaded} data={user} />
            <Email status={isLoaded} data={user} />
          </div>
        </div>
        <div className="flex-col-3">
